  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Gaborone Trading House (Pty) Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Gaborone Trading House (Pty) Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest reservation-ineligible-is-held-and-unoverridable
  (testing "0% citizen ownership on a reserved category (Public Procurement Act, 2021 s.76(1)/s.78(1)) -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:reservation-ineligible} (-> (store/ledger db) last :basis)))
...
(deftest tin-registration-unverified-is-held-and-unoverridable
  (testing "unverified Botswana Unified Revenue Service TIN registration when required -> HARD hold"
    (let [[db actor] (fresh)
