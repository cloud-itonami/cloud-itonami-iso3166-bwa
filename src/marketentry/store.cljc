(ns marketentry.store
  "SSoT for the Republic of Botswana (BWA) market-entry compliance
...
(defn demo-data
  ...
  `:reserved-category?` / `:reservation-exempt?` /
  `:citizen-ownership-pct` / `:joint-venture?` /
  `:citizen-subcontractor-association?` are ground truth for the
  flagship ORDERED TIER-CLASSIFICATION check (Public Procurement Act,
  2021 s.76(1) default reservation + s.78(1) descending preference
  scale -- is the engagement's own declared ownership/arrangement
...
(defn- engagement->tx [{:keys [id operator procurement-channel base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? reserved-category?)                     (assoc :engagement/reserved-category? reserved-category?)
    (some? reservation-exempt?)                    (assoc :engagement/reservation-exempt? reservation-exempt?)
    citizen-ownership-pct                          (assoc :engagement/citizen-ownership-pct citizen-ownership-pct)
    (some? joint-venture?)                         (assoc :engagement/joint-venture? joint-venture?)
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :procurement-channel (:engagement/procurement-channel m)
     ...
     :reserved-category? (boolean (:engagement/reserved-category? m))
     :reservation-exempt? (boolean (:engagement/reservation-exempt? m))
     :citizen-ownership-pct (:engagement/citizen-ownership-pct m)
     :joint-venture? (boolean (:engagement/joint-venture? m))
