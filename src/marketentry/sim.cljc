...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (BWA, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up reservation-ineligible) ==")
    ...
    (println "== filing/submit eng-4 (0% citizen ownership on a reserved category -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up TIN-unregistered) ==")
